...
class RealtimeService {
  ...
  static Stream<List<Putnik>> putniciSaDugovimStream() {
    try {
      return _putnici.where('duguje', isGreaterThan: 0).orderBy('duguje', descending: true).snapshots().map((snapshot) {
        return snapshot.docs.map((doc) {
          final data = doc.data() as Map<String, dynamic>;
          return Putnik.fromJson({...data, 'id': doc.id});
        }).toList();
  ...
  static Stream<Map<String, dynamic>> statistikaDanasStream() {
    try {
      ...
      return _dnevniPutnici
          .where('datum', isGreaterThanOrEqualTo: Timestamp.fromDate(todayStart))
