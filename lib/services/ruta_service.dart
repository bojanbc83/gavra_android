...
class RutaService {
  ...
  Future<Ruta?> createRuta(Ruta ruta) async {
    try {
      ...
      if (validation.isNotEmpty) {
        dlog('❌ [RUTA SERVICE] Validacija neuspešna: ${validation.values.join(', ')}');
        return null;
      }

      // Proveri duplikate
      final existingRute = await _checkForDuplicates(ruta.polazak, ruta.dolazak, ruta.naziv);
  ...
  Future<List<Ruta>> _checkForDuplicates(String polazak, String dolazak, String naziv) async {
    try {
      final response = await SupabaseSafe.run(
        () => _supabase.from('rute').select().or('and(polazak.eq.$polazak,dolazak.eq.$dolazak),naziv.eq.$naziv'),
        fallback: <dynamic>[],
      );
  ...
  Future<Ruta?> updateRuta(String id, Map<String, dynamic> updates) async {
    try {
      ...
      if (validation.isNotEmpty) {
        dlog('❌ [RUTA SERVICE] Ažurirana ruta nije validna: ${validation.values.join(', ')}');
        return null;
      }
  ...
  Future<bool> deleteRuta(String id) async {
    try {
      ...
      if (hasDependencies) {
        dlog('❌ [RUTA SERVICE] Ne može se obrisati ruta $id - ima povezane putnike');
        return false;
      }
      ...
      final response = await SupabaseSafe.run(
        () {
          ...
          if (query != null && query.isNotEmpty) {
            q = q.or('naziv.ilike.%$query%,polazak.ilike.%$query%,dolazak.ilike.%$query%,opis.ilike.%$query%');
          }
          ...
          if (minVremeMinuti != null) {
            q = q.gte('prosecno_vreme', minVremeMinuti * 60); // Konvertuj u sekunde
          }
          if (maxVremeMinuti != null) {
            q = q.lte('prosecno_vreme', maxVremeMinuti * 60); // Konvertuj u sekunde
          }
  ...
  Future<List<Ruta>> getRuteIzmedju(String polazak, String destinacija, {bool sameAktivan = true}) async {
    try {
      final response = await SupabaseSafe.run(
        () {
      ...
      dlog('❌ [RUTA SERVICE] Greška pri dohvatanju ruta između $polazak i $destinacija: $e');
      return [];
    }
  }
  ...
  Future<List<Ruta>> batchCreateRute(List<Ruta> rute) async {
    try {
      ...
      for (final ruta in rute) {
        final validation = ruta.validateFull();
        if (validation.isNotEmpty) {
          dlog('❌ [RUTA SERVICE] Batch validacija neuspešna za ${ruta.naziv}: ${validation.values.join(', ')}');
          return [];
        }
      }
  ...
  Future<List<Ruta>> batchUpdateRute(List<Ruta> rute) async {
    try {
      ...
      dlog('✅ [RUTA SERVICE] Batch ažurirano ${results.length}/${rute.length} ruta');
      return results;
    } catch (e) {
  ...
  Future<Map<String, dynamic>> getRuteStatistics() async {
    try {
      ...
      final cached =
          await CacheService.getFromDisk<Map<String, dynamic>>(cacheKey, maxAge: const Duration(minutes: 30));
      if (cached != null) {
      ...
      final ukupnaUdaljenost =
          aktivneRute.where((r) => r.udaljenostKm != null).fold<double>(0, (sum, r) => sum + r.udaljenostKm!);

      final prosecnaUdaljenost = aktivneRute.where((r) => r.udaljenostKm != null).isNotEmpty
          ? ukupnaUdaljenost / aktivneRute.where((r) => r.udaljenostKm != null).length
          : 0.0;

      final ukupnoVreme =
          aktivneRute.where((r) => r.prosecnoVreme != null).fold<int>(0, (sum, r) => sum + r.prosecnoVreme!.inMinutes);

      final prosecnoVreme = aktivneRute.where((r) => r.prosecnoVreme != null).isNotEmpty
          ? ukupnoVreme / aktivneRute.where((r) => r.prosecnoVreme != null).length
          : 0.0;
      ...
      final stats = {
        ...
        'procenat_aktivnih': rute.isNotEmpty ? (aktivneRute.length / rute.length * 100).round() : 0,
        'ukupna_udaljenost_km': ukupnaUdaljenost,
        'prosecna_udaljenost_km': double.parse(prosecnaUdaljenost.toStringAsFixed(1)),
        'ukupno_vreme_minuti': ukupnoVreme,
        'prosecno_vreme_minuti': double.parse(prosecnoVreme.toStringAsFixed(1)),
        'najčešći_polazak':
            polasciCount.isNotEmpty ? polasciCount.entries.reduce((a, b) => a.value > b.value ? a : b).key : null,
        'najčešći_dolazak':
            dolasciCount.isNotEmpty ? dolasciCount.entries.reduce((a, b) => a.value > b.value ? a : b).key : null,
  ...
  Future<Map<String, dynamic>> getRutaStatistics(String rutaId) async {
    try {
      ...
      dlog('❌ [RUTA SERVICE] Greška pri dobijanju statistika za rutu $rutaId: $e');
      return {};
    }
  }
  ...
  Future<String> exportRuteToCSV({bool sameAktivne = true}) async {
    try {
      ...
      // Header
      csvLines.add('ID,Naziv,Polazak,Dolazak,Opis,Udaljenost (km),Prosečno vreme (min),Aktivan,Kreiran');
  ...
  Future<void> cleanupOldInactiveRute({int daysToKeep = 365}) async {
    try {
      ...
      dlog('✅ [RUTA SERVICE] Očišćene stare neaktivne rute starije od $cutoffDateStr');
    } catch (e) {
