...
class GPSLokacijaService {
  ...
  GPSLokacijaService(); // Firebase migration
  ...
  Future<GPSLokacija> snimiLokaciju(GPSLokacija lokacija) async {
    final response = await _supabase.from('gps_lokacije').insert(lokacija.toMap()).select().single();
  ...
  Future<List<GPSLokacija>> getPoslednjeLokacijeSvihVozila() async {
    ...
    final response = await _supabase.rpc<List<Map<String, dynamic>>>('get_poslednje_lokacije_vozila').select();
  ...
  Future<int> ocistiStareLokacije(int dana) async {
    ...
    final response =
        await _supabase.from('gps_lokacije').update({'aktivan': false}).lt('vreme', cutoffDate.toIso8601String());
  ...
  Stream<List<GPSLokacija>> gpsLokacijeStreamZaVozilo(String voziloId) {
    return _supabase.from('gps_lokacije').stream(primaryKey: ['id']).order('vreme').map(
          (data) => data
              .where(
                (lokacija) => lokacija['vozilo_id'] == voziloId && lokacija['aktivan'] == true,
              )
  ...
  Stream<List<GPSLokacija>> get sveGPSLokacijeStream {
    return _supabase.from('gps_lokacije').stream(primaryKey: ['id']).eq('aktivan', true).order('vreme').map((data) {
