...
class PutnikService {
  ...
  static bool _isDuplicateAction(String actionKey) {
    ...
    if (lastAction != null) {
      ...
      if (timeDifference < _duplicatePreventionDelay) {        return true;
      }
    }
  ...
  Future<List<Putnik>> getAllPutniciFromBothTables({String? targetDay}) async {
    ...
    try {
      ...
      final danas = datum.toIso8601String().split('T')[0];// ✅ ISPRAVKA: Koristi istu logiku kao danas_screen - povlači iz putovanja_istorija
      ...
      final danKratica = _getDayAbbreviationFromName(targetDate);// Explicitly request polasci_po_danu and common per-day columns
      const mesecniFields = '*,'
          'polasci_po_danu';
      ...
      final mesecniResponse = await supabase
          ...
          .order('created_at', ascending: false);      for (final data in mesecniResponse) {
  ...
  Future<void> dodajPutnika(Putnik putnik) async {
    try {
      ...
      if (putnik.adresa != null && putnik.adresa!.isNotEmpty) {
      ...
      }      if (putnik.mesecnaKarta == true) {// ✅ PROVERAVA DA LI MESEČNI PUTNIK VEĆ POSTOJI
        ...
        if (existingPutnici.isEmpty) {          throw Exception('MESEČNI PUTNIK NE POSTOJI!\n\n'
        ...
        try {
          ...
          await _dnevniPutnikService.createDnevniPutnik(dnevniPutnik);        } catch (e) {// FALLBACK: Koristi legacy putovanja_istorija pristup          final insertData = putnik.toPutovanjaIstorijaMap();          final insertRes = await SupabaseSafe.run(
            () => supabase.from('putovanja_istorija').insert(insertData),
            fallback: <dynamic>[],
          );
          if (insertRes == null) {          } else {          }
        }
      }
      ...
      if (putnik.dan == todayName) {        RealtimeNotificationService.sendRealtimeNotification(
          'Novi putnik',
        ...
        );      } else {      }    } catch (e) {      rethrow; // Ponovno baci grešku da je home_screen može uhvatiti
    }
  }

  /// ✅ KOMBINOVANI STREAM - MESEČNI + DNEVNI PUTNICI
  Stream<List<Putnik>> streamKombinovaniPutnici() {    final danasKratica = _getFilterDayAbbreviation(DateTime.now().weekday);
    final danas = DateTime.now().toIso8601String().split('T')[0];
    ...
    return CombineLatestStream.combine2(
      ...
      for (final item in mesecniData) {
        try {
          ...
          if (radniDani.toLowerCase().contains(danasKratica.toLowerCase())) {
            ...
            sviPutnici.addAll(mesecniPutnici);          } else {          }
        } catch (e) {        }
      }
      ...
      try {
        ...
        }).toList();        for (final item in dnevniFiltered) {
          try {
            final putnik = Putnik.fromPutovanjaIstorija(item as Map<String, dynamic>);
            sviPutnici.add(putnik);          } catch (e) {          }
        }
      } catch (e) {      }
  ...
  Future<void> obrisiPutnika(dynamic id) async {// Određi tabelu na osnovu ID-ja
  ...
  Future<void> oznaciPokupljen(dynamic id, String currentDriver) async {
    ...
    if (_isDuplicateAction(actionKey)) {      return;
    }
    ...
    if (tabela == 'mesecni_putnici') {
      ...
      try {
        await MesecniPutnikService.sinhronizujBrojPutovanjaSaIstorijom(id);      } catch (e) {      }
    } else {
    ...
    if (tabela == 'putovanja_istorija' && response['mesecni_putnik_id'] != null) {
      try {        await MesecniPutnikService.sinhronizujBrojPutovanjaSaIstorijom(
          response['mesecni_putnik_id'] as String,
        );      } catch (syncError) {// Nastavi dalje - sinhronizacija nije kritična
      }
    }
  ...
  Future<void> oznaciPlaceno(
    ...
    // Određi tabelu na osnovu ID-ja    final tabela = await _getTableForPutnik(id);// Prvo dohvati podatke putnika za notifikaciju    final response = await SupabaseSafe.run(
      () => supabase.from(tabela).select().eq('id', id as String).single(),
    );// 📝 DODAJ U UNDO STACK (sigurno mapiranje)
    final undoPayment = response == null ? <String, dynamic>{} : Map<String, dynamic>.from(response as Map);
    _addToUndoStack('payment', id, undoPayment);    if (tabela == 'mesecni_putnici') {
      ...
      }).eq('id', id as String);    }// (Uklonjeno slanje notifikacije za plaćanje)
  }
  ...
  Future<void> otkaziPutnika(
    ...
    String? selectedGrad,
      ...
      final cancelName = (respMap['putnik_ime'] ?? respMap['ime']) ?? '';// 📝 DODAJ U UNDO STACK
      _addToUndoStack('cancel', id, respMap);      if (tabela == 'mesecni_putnici') {
        // 🆕 NOVI PRISTUP: Za mesečne putnike kreiraj zapis u putovanja_istorija za konkretan dan        final danas = DateTime.now().toIso8601String().split('T')[0];
        final polazak = selectedVreme ?? '5:00'; // Koristi proslijećeno vreme ili default
        final grad = selectedGrad ?? 'Bela Crkva'; // Koristi proslijećeni grad ili default// Kreiraj zapis otkazivanja za današnji dan
      ...
      // 📬 POŠALJI NOTIFIKACIJU ZA OTKAZIVANJE (za tekući dan)      try {
        final now = DateTime.now();
        final dayNames = ['Pon', 'Uto', 'Sre', 'Čet', 'Pet', 'Sub', 'Ned'];
        final todayName = dayNames[now.weekday - 1];
        ...
        if (danLowerCase.contains(todayLowerCase) || putnikDan == todayName) {          RealtimeNotificationService.sendRealtimeNotification(
            'Otkazan putnik',
      ...
      } catch (notifError) {// Nastavi dalje - notifikacija nije kritična
      }

      // 📊 AUTOMATSKA SINHRONIZACIJA BROJA OTKAZIVANJA (NOVO!)
      if (tabela == 'putovanja_istorija' && (respMap['mesecni_putnik_id'] != null)) {
        try {          await MesecniPutnikService.sinhronizujBrojOtkazivanjaSaIstorijom(
  ...
  Future<void> resetPutnikCard(String imePutnika, String currentDriver) async {
    try {      if (!VozacBoja.isValidDriver(currentDriver)) {        throw Exception('Samo validni vozači mogu da resetuju kartice');
      }// Pokušaj reset u mesecni_putnici tabeli
      try {        final mesecniResponse =
            await supabase.from('mesecni_putnici').select().eq('putnik_ime', imePutnika).maybeSingle();

        if (mesecniResponse != null) {          await supabase.from('mesecni_putnici').update({
          ...
          try {            final putnikId = mesecniResponse['id'] as String;
            await MesecniPutnikService.sinhronizujBrojOtkazivanjaSaIstorijom(
              putnikId,
            );// 📊 TAKOĐE sinhronizuj broj putovanja (NOVO!)            await MesecniPutnikService.sinhronizujBrojPutovanjaSaIstorijom(
              putnikId,
            );          } catch (syncError) {          }          return;
        }      } catch (e) {// Ako nema u mesecni_putnici, nastavi sa putovanja_istorija
      }

      // Pokušaj reset u putovanja_istorija tabeli      final danas = DateTime.now().toIso8601String().split('T')[0];
      final putovanjaResponse = await supabase
          .from('putovanja_istorija')
          .select()
          .eq('putnik_ime', imePutnika)
      ...
      if (putovanjaResponse != null) {        await supabase
            .from('putovanja_istorija')
            .update({
            ...
            .eq('putnik_ime', imePutnika)
            .eq('datum', danas);      } else {      }
    } catch (e) {// Greška pri resetovanju kartice
      rethrow;
    }
  }
  ...
  Future<void> resetPokupljenjaNaPolazak(
    ...
    try {      if (!VozacBoja.isValidDriver(currentDriver)) {        return;
      }
      ...
      try {
        final mesecniPutnici = await supabase
            .from('mesecni_putnici')
            .select('id, putnik_ime, polasci_po_danu, vreme_pokupljenja') // ✅ FIXED: Koristi vreme_pokupljenja
            .eq('aktivan', true)
            .not('vreme_pokupljenja', 'is', null); // ✅ FIXED: Koristi vreme_pokupljenja
        ...
        for (final putnik in mesecniPutnici) {
          ...
          if (razlika > 3) {            await supabase.from('mesecni_putnici').update({
      ...
      try {
        ...
        for (final putnik in dnevniPutnici) {
          ...
          // Jednostavno resetuj sve pokupljene putnike kada se menja vreme          await supabase.from('putovanja_istorija').update({
            'status': 'nije_se_pojavio',
            'cena': 0,
            // 'vreme_akcije': DateTime.now().toIso8601String(), // UKLONITI - kolona ne postoji
          }).eq('id', putnik['id'] as String);        }      } catch (e) {      }    } catch (e) {    }
  }
