...
import 'services/simple_usage_monitor.dart'; // DODANO za monitoring
...
class _MyAppState extends State<MyApp> {
  ...
  void initState() {
    ...
    // 🚀 ODLOŽENA INICIJALIZACIJA - pokreni servise nakon što se app učita
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (!mounted) return;

      // Pusti app da se učita prvo, pa tek onda pokreni servise
      await Future<void>.delayed(const Duration(seconds: 2));
      if (!mounted) return;

      // Pokreni servise u pozadini bez čekanja
  ...
  Future<void> _initializeApp() async {
    try {
      // 🚀 BRZA INICIJALIZACIJA - samo osnovne stvari
      // Bez mrežnih poziva da se app odmah pokrene
  ...
  // 🚀 BACKGROUND INICIJALIZACIJA SERVISA - ne blokira UI
  void _initializeServicesInBackground() async {
    try {
      // INICIJALIZUJ NOTIFIKACIJE SEKVENCIJALNO
      try {
      ...
      // Ignoriši sve greške da ne remeti app
    }
  }
