  ...
  group('🔧 Integracija sa stvarnim aplikacijskim podacima', () {
    ...
    test('🔍 Provjera da svi vozači iz PasswordService postoje u VozacMapping', () {
      ...
      for (final driverName in passwordDrivers) {
        ...
        expect(uuid, isNotNull, reason: 'Vozač $driverName mora imati UUID u VozacMappingService');
      }
    });

    test('🔍 Provjera da svi vozači iz VozacMapping imaju šifre u PasswordService', () {
      ...
      for (final driverName in mappingDrivers) {
        expect(passwordDrivers, contains(driverName), 
            reason: 'Vozač $driverName iz VozacMapping mora imati šifru u PasswordService');
      }
    });
    ...
    test('📊 Sažetak svih vozača', () {
      ...
      for (final vozac in vozaci) {
        ...
        print('   Validno UUID: ${VozacMappingService.isValidVozacUuid(uuid!)}');
        print('   Fallback test: ${VozacMappingService.getVozacImeWithFallback(uuid)}');
        print('');
      }
    });
  });
